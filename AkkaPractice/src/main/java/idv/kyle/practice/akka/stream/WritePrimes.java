package idv.kyle.practice.akka.stream;
...
public class WritePrimes {
	...
	final Source<Integer, NotUsed> primeSource = Source
		.from(new RandomIterable(maxRandomNumberSize)).
	...
	// write to file sink
	Sink<ByteString, CompletionStage<IOResult>> output = FileIO
		.toFile(new File("target/primes.txt"));
	Sink<Integer, CompletionStage<IOResult>> slowSink = Flow
		.of(Integer.class).map(i -> {
	...
	// console output sink
	Sink<Integer, CompletionStage<Done>> consoleSink = Sink
		.<Integer> foreach(System.out::println);

	// connect the graph, materialize and retrieve the completion Future
	final Graph<ClosedShape, CompletionStage<IOResult>> graph = GraphDSL
		.create(slowSink, (b, sink) -> {
		    final UniformFanOutShape<Integer, Integer> bcast = b
			    .add(Broadcast.<Integer> create(2));
		    b.from(b.add(primeSource)).viaFanOut(bcast).to(sink)
			    .from(bcast).to(b.add(consoleSink));
		    return ClosedShape.getInstance();
		});
	final CompletionStage<IOResult> future = RunnableGraph.fromGraph(graph)
		.run(materializer);

	future.handle((ioResult, failure) -> {
	    if (failure != null)
		System.err.println("Failure: " + failure);
	    else if (!ioResult.wasSuccessful())
		System.err.println(
			"Writing to file failed " + ioResult.getError());
	    else
		System.out.println(
			"Successfully wrote " + ioResult.getCount() + " bytes");
	    system.terminate();
